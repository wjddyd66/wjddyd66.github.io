...
\end{matrix} = 
\begin{matrix}
\sigmoid(W_i[x_t,h_{t-1}]+b_i) \\
\sigmoid(W_f[x_t,h_{t-1}]+b_f) \\
\sigmoid(W_o[x_t,h_{t-1}]+b_o) \\
...
\end{matrix} = 
\begin{matrix}
\sigmoid(W_i[x_t,D(h_{t-1})]+b_i) \\
\sigmoid(W_f[x_t,D(h_{t-1})]+b_f) \\
\sigmoid(W_o[x_t,D(h_{t-1})]+b_o) \\
