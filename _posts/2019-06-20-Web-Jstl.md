...
###  JSTL 기본
1. 선언: <code><%@ taglib prefix="사용할 변수명" uri="http://java.sun.com/jsp/jstl/core"%></code>
2. 입력: <code><c:set var="변수명" value="값" scope="범위" /></code>
3. 출력: <code><c:out value="${변수명}" /></code>
4. 조건문
 - 범위: <code><c:choose></code>
 - if: <code><c:when test="조건"></code> 
 - else: <code><c:otherwise></code>
5. 반복문: <code><c:forEach var="변수명" begin="시작값" end="끝값" step="증가값"></code>
6. 문자열 분할: <code><c:forTokens var="변수명" items="분할할 변수명" delims="문자 구분 기준" begin="시작 배열의 Index" end="끝 배열의 Index" step="Index 증가값"></code>
7. Try-Catch: <code><c:catch var="변수명"> 실행할 내용 </c:catch></code>  
