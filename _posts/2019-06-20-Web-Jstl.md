...
###  JSTL 기본
1. 선언: <pre><%@ taglib prefix="사용할 변수명" uri="http://java.sun.com/jsp/jstl/core"%></pre>
2. 입력: <pre><c:set var="변수명" value="값" scope="범위" /></pre>
3. 출력: <pre><c:out value="${변수명}" /></pre>
4. 조건문
 - 범위: <pre><c:choose></pre>
 - if: <pre><c:when test="조건"></pre> 
 - else: <pre><c:otherwise></pre>
5. 반복문: <pre><c:forEach var="변수명" begin="시작값" end="끝값" step="증가값"></pre>
6. 문자열 분할: <pre><c:forTokens var="변수명" items="분할할 변수명" delims="문자 구분 기준" begin="시작 배열의 Index" end="끝 배열의 Index" step="Index 증가값"></pre>
7. Try-Catch: <pre><c:catch var="변수명"> 실행할 내용 </c:catch></pre>  
