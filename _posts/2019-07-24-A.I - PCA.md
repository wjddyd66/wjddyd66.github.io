...
<p>$$ = (A^{T}u_2)^{T}u_1 = (Au_2)^{T}u_1 = \lamda_2u_2^{T}u_1$$</p>
<p>$$ (\lamda_1 - \lamda_2)u_2^{T}u_1 = 0  $$</p>
...
<p>$$A = U \sum^ V^{T}$$</p>
<p>$$A: m x n 행렬$$</p>
<p>$$U: m x m 직교행렬(AA^{T} = U(\sum \sum^{T})U^{T})$$</p>
<p>$$V: n x n 직교행렬(A^{T}A = V(\sum^{T} \sum)V^{T})$$</p>
<p>$$\sum: m x n 직사각 대각행렬$$</p>
위의 조건을 하나씩 살펴보자  
1)<span>$$U: m x m 직교행렬(AA^{T} = U(\sum \sum^{T})U^{T})$$ </span>  
 - <span>$$AA{T}는 대칭행렬이므로 <span>**대칭행렬과 고유값 분해**</span>로 인하여 orthogoanl백터로서 나타낼 수 있다.$$</span>
 - <span>$$A{T} = V \sum^{T} U <span>
 - <span>$$AA{T} = U \sum (V^{T} V) \sum^{T} U <span>
 - <span>$$AA{T} = U \sum E \sum^{T} U <span>
 - <span>$$AA{T} = U (\sum  \sum^{T}) U <span>

2) V도 1)과 같이 증명하면 똑같이 나오게 된다.  
3) <span>$$\sum: m x n 직사각 대각행렬$$</span>  
